...
def test_simple_consultation():
    ...
    try:
        ...
        result = consilium.consult(
            "Explain the difference between REST and GraphQL APIs", use_smart_routing=True, check_health=True
        )
        ...
        if result.get("director_decision"):
            decision = (
                result["director_decision"][:200] + "..."
                if len(result["director_decision"]) > 200
                else result["director_decision"]
            )
