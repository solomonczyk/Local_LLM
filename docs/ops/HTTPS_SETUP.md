# 🔒 Настройка HTTPS для Agent System

## Local HTTPS (self-signed, Docker)
```bash
docker compose --profile https up -d nginx-https
# UI: https://localhost:8443
```
Note: run ./generate_ssl.sh (requires openssl) before enabling HTTPS.

## 🎯 Цель
Настроить бесплатный SSL сертификат от Let's Encrypt для безопасного доступа к агентской системе по HTTPS.

## 🚀 Быстрая настройка (2 минуты)

### Вариант 1: Автоматический скрипт
```bash
# Запустите на локальной машине
chmod +x quick-https.sh
./quick-https.sh
```

### Вариант 2: Ручная настройка

1. **Загрузите файлы на сервер:**
```bash
scp nginx-https.conf setup-https.sh root@152.53.227.37:/root/
```

2. **Подключитесь к серверу:**
```bash
ssh root@152.53.227.37
```

3. **Запустите настройку:**
```bash
cd /root
chmod +x setup-https.sh
sudo ./setup-https.sh
```

## 🔧 Что делает скрипт

1. **Устанавливает certbot** для получения SSL сертификатов
2. **Получает бесплатный SSL сертификат** от Let's Encrypt для доменов:
   - `152.53.227.37.nip.io`
   - `agent.152.53.227.37.nip.io`
   - `api.152.53.227.37.nip.io`
   - `llm.152.53.227.37.nip.io`
   - `tools.152.53.227.37.nip.io`
3. **Обновляет nginx конфигурацию** для HTTPS
4. **Настраивает автоматическое обновление** сертификата
5. **Перезапускает сервисы** с новой конфигурацией

## 🌐 Результат

После настройки будут доступны HTTPS ссылки:

| Сервис | HTTPS URL | Описание |
|--------|-----------|----------|
| **🖥️ Agent UI** | https://agent.152.53.227.37.nip.io | Веб-интерфейс |
| **🤖 LLM API** | https://api.152.53.227.37.nip.io | API языковой модели |
| **🔧 Tools API** | https://tools.152.53.227.37.nip.io | API инструментов |

## 🔒 Безопасность

### Что добавляется:
- ✅ **SSL/TLS шифрование** всего трафика
- ✅ **HTTP → HTTPS редирект** автоматически
- ✅ **Security headers** (HSTS, XSS Protection, etc.)
- ✅ **Rate limiting** для защиты от DDoS
- ✅ **Современные SSL протоколы** (TLS 1.2, 1.3)

### SSL сертификат:
- 🆓 **Бесплатный** от Let's Encrypt
- ⏰ **Действует 90 дней** с автообновлением
- 🔄 **Автоматическое обновление** через cron
- 🌍 **Доверенный** всеми браузерами

## 🛠️ Устранение проблем

### Если сертификат не создался:
```bash
# Проверьте DNS
nslookup 152.53.227.37.nip.io

# Проверьте доступность порта 80
sudo netstat -tlnp | grep :80

# Остановите другие веб-сервисы
sudo systemctl stop apache2 nginx
```

### Если nginx не запускается:
```bash
# Проверьте конфигурацию
sudo nginx -t

# Посмотрите логи
sudo docker logs agent-nginx
```

### Проверка SSL сертификата:
```bash
# Проверьте сертификат
sudo certbot certificates

# Тест обновления
sudo certbot renew --dry-run
```

## 🔄 Обновление сертификата

Сертификат обновляется автоматически, но можно обновить вручную:

```bash
# Обновить сертификат
sudo certbot renew

# Перезапустить nginx
docker compose restart nginx
```

## 📋 Проверочный список

После настройки проверьте:

- [ ] https://agent.152.53.227.37.nip.io открывается без ошибок
- [ ] Браузер показывает зеленый замок 🔒
- [ ] HTTP автоматически редиректит на HTTPS
- [ ] API работает по HTTPS
- [ ] SSL сертификат действителен (не истек)

## 🆘 Откат к HTTP

Если что-то пошло не так, можно вернуться к HTTP:

```bash
# Остановить контейнеры
docker compose down

# Вернуть старую конфигурацию
cp nginx.conf.backup nginx.conf

# Запустить без SSL
docker compose up -d
```

## 💡 Альтернативы

### Cloudflare (рекомендуется для продакшена)
1. Зарегистрируйте домен
2. Добавьте в Cloudflare
3. Настройте DNS записи
4. Получите бесплатный SSL от Cloudflare

### Self-signed сертификат (для тестирования)
```bash
# Создать самоподписанный сертификат
openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
  -keyout selfsigned.key -out selfsigned.crt
```

---

**Готово!** 🎉 Теперь ваша агентская система работает по безопасному HTTPS протоколу.