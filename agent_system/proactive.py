...
class ProactiveAgent:
    ...
    def __init__(self):
        self.suggestion_patterns = {
            ...
            "file_creation": [
                r"создай|напиши|сделай.*файл",
                r"нужен.*файл",
                r"можешь.*создать"
            ],
            "code_review": [
                r"проверь|посмотри.*код",
                r"есть.*ошибк",
    ...
    def _analyze_current_directory(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "config_files": len(list(current_dir.glob("*.json")) + list(current_dir.glob("*.yaml")) + list(current_dir.glob("*.yml"))),
            ...
            "recent_files": sorted([f for f in files if f.is_file()], key=lambda x: x.stat().st_mtime, reverse=True)[:5]
        }
    ...
    def generate_clarifying_questions(self, user_message: str) -> List[str]:
        ...
        if any(word in message_lower for word in ["не работает", "ошибка", "проблема"]):
            questions.extend([
                ...
                "📋 В каком файле или функции возникла проблема?"
            ])
