...
def main():
    ...
    services_to_start = [
        {
            "name": "tool_server",
            "command": ["python", "-m", "agent_system.tool_server", "--port", "8011"],
            "delay": 0
        },
        {
            "name": "llm_server", 
            "command": ["python", "serve_enhanced.py", "--port", "8010"],
            "delay": 3
        },
        {
            "name": "ui_server",
            "command": ["python", "ui.py", "--server_port", "7864"],
            "delay": 6
        }
    ]
    ...
    for service_config in services_to_start:
        ...
        if not success:
            print(f"❌ Failed to start {service_config['name']}, exiting...")
            manager.stop_all_services()
            sys.exit(1)
