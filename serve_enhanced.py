...
🔄 Используется: {data['used_gb']} ГБ ({data['percent_used']}%){context_info}"""
    ...
    # Работа с базами данных
        ...
        if any(word in text_lower for word in ["подключи", "connect", "настрой"]):
            return """🗄️ Для подключения к PostgreSQL нужны параметры:
...
⚠️ **Безопасность:** Пароли не сохраняются в файлах конфигурации.""" + context_info

        elif any(word in text_lower for word in ["таблицы", "схема", "структура"]):
            return """📊 Для просмотра структуры БД могу показать:
...
💡 Сначала нужно настроить подключение к БД.""" + context_info

        elif any(word in text_lower for word in ["select", "выбери", "найди", "покажи данные"]):
            return """🔍 Для выполнения SQL запросов:
...
🔒 Все запросы проверяются на безопасность.""" + context_info

        else:
            return """🗄️ Работа с PostgreSQL базами данных:
...
🔒 Система обеспечивает безопасность через валидацию запросов.""" + context_info
...
💡 Хотите, чтобы я создал базовую структуру API для вас?""" + context_info
